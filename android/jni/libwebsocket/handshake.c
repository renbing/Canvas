...
interpret_key(const char *key, unsigned long *result)
{
	...
	if (rem) {
		LOG("nonzero handshake remainder\n");
		return -1;
	}
...
handshake_0405(struct libwebsocket_context *context, struct libwebsocket *wsi)
{
	...
	if (wsi->utf8_token[WSI_TOKEN_KEY].token_len >=
		...
		LOG("Client sent handshake key longer "
	...
	accept_len = lws_b64_encode_string((char *)hash, 20, accept_buf,
							     sizeof accept_buf);
	if (accept_len < 0) {
		LOG("Base64 encoded hash too long\n");
		goto bail;
	}
	...
	if (wsi->ietf_spec_revision == 4) {
		...
		n = libwebsockets_get_random(wsi->protocol->owning_server,
								      hash, 16);
		if (n != 16) {
			LOG("Unable to read random device %s %d\n",
		...
		if (nonce_len < 0) {
			LOG("Failed to base 64 encode the nonce\n");
			if (wsi->user_space)
				free(wsi->user_space);
			goto bail;
		}
	...
	if (wsi->utf8_token[WSI_TOKEN_EXTENSIONS].token_len) {
		...
		LOG("wsi->utf8_token[WSI_TOKEN_EXTENSIONS].token = %s\n", wsi->utf8_token[WSI_TOKEN_EXTENSIONS].token);
		...
		while (more) {
			...
			while (ext && ext->callback) {
				...
				wsi->count_active_extensions++;
				LOG("wsi->count_active_extensions <- %d", wsi->count_active_extensions);
						...
						unsigned char * buf, size_t len)
{
	...
	case WSI_STATE_HTTP_HEADERS:
		...
		LOG("seem to be serving, mode is %d\n", wsi->mode);

		LOG("libwebsocket_parse sees parsing complete\n");
		...
		if (!wsi->protocol) {
			LOG("NULL protocol coming on libwebsocket_read\n");
		}
		...
		if ((wsi->protocol->callback)(wsi->protocol->owning_server, wsi,
			...
			LOG("User code denied connection\n");
			goto bail;
		}
		...
		default:
			LOG("Unknown client spec version %d\n",
						       wsi->ietf_spec_revision);
			goto bail;
		}

		LOG("accepted v%02d connection\n",
						       wsi->ietf_spec_revision);
