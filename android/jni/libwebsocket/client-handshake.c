...
struct libwebsocket * __libwebsocket_client_connect_2(
	...
	LOG("__libwebsocket_client_connect_2\n");
...
/*Proxy-authorization: basic aGVsbG86d29ybGQ= */
	...
	LOG("__libwebsocket_client_connect_2: address %s", wsi->c_address);

	server_hostent = gethostbyname(wsi->c_address);
	if (server_hostent == NULL) {
		LOG("Unable to get host name from %s\n",
								wsi->c_address);
		goto oom4;
	}
	...
	if (context->http_proxy_port) {
		...
		if (n < 0) {
			...
			close(wsi->sock);
#endif
			LOG("ERROR writing to proxy socket\n");
			goto bail1;
		}
				  ...
				  void *callback_arg)
{
	...
	if (ssl_connection) {
		LOG("libwebsockets not configured for ssl\n");
		return NULL;
	}
#endif
	...
	case 13:
		...
		break;
	default:
		LOG("Client ietf version %d not supported\n", wsi->ietf_spec_revision);
		goto oom4;
	}
	...
	if (handled) {
		LOG("libwebsocket_client_connect: "
							 "ext handling conn\n");
	...
	LOG("libwebsocket_client_connect: direct conn\n");
