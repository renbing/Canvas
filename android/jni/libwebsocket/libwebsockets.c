...
delete_from_fd(struct libwebsocket_context *context, int fd)
{
	...
	LOG("Failed to find fd %d requested for "
						   "delete in hashtable\n", fd);
	return 1;
}
			 ...
			 struct libwebsocket *wsi, enum lws_close_status reason)
{
	...
	if (old_state == WSI_STATE_ESTABLISHED &&
		...
		if (!n) {
			...
			LOG("sent close indication, awaiting ack\n");
...
just_kill_connection:
	...
	LOG("libwebsocket_close_and_free_session: just_kill_connection\n");
	...
	if (wsi->protocol && wsi->protocol->callback &&
		...
		LOG("calling back CLOSED\n");
		wsi->protocol->callback(context, wsi, LWS_CALLBACK_CLOSED,
						      wsi->user_space, NULL, 0);
	} else {
		LOG("not calling back closed due to old_state=%d\n", old_state);
	}
		...
		for (n = 0; n < wsi->count_active_extensions; n++) {
			m = wsi->active_extensions[n]->callback(
				...
				wsi->active_extensions[n], wsi,
				   ...
				   wsi->active_extensions_user[n], &eff_buf, 0);
			if (m < 0) {
				LOG("extension reports fatal error\n");
				return -1;
			}
			if (m)
				/*
...
libwebsocket_create_new_server_wsi(struct libwebsocket_context *context)
{
	...
	if (new_wsi == NULL) {
		LOG("Out of memory for new connection\n");
		return NULL;
	}
		...
		struct libwebsocket *wsi, char *pkt)
{
	...
	if (n != 16) {
		LOG("Unable to read from random dev %s\n",
	...
	if (wsi->ietf_spec_revision == 0) {
		...
		max_1 = 4294967295u / spaces_1;
		max_2 = 4294967295u / spaces_2;
		...
		struct libwebsocket *wsi)
{
	...
	if (wsi->ietf_spec_revision == 0) {
		if (!wsi->utf8_token[WSI_TOKEN_HTTP].token_len ||
			...
			wsi->c_protocol != NULL)) {
			LOG("libwebsocket_client_handshake "
	 ...
	 */
#if 0
	LOG("WSI_TOKEN_HTTP: %d\n", wsi->utf8_token[WSI_TOKEN_HTTP].token_len);
	LOG("WSI_TOKEN_UPGRADE: %d\n", wsi->utf8_token[WSI_TOKEN_UPGRADE].token_len);
	LOG("WSI_TOKEN_CONNECTION: %d\n", wsi->utf8_token[WSI_TOKEN_CONNECTION].token_len);
	LOG("WSI_TOKEN_ACCEPT: %d\n", wsi->utf8_token[WSI_TOKEN_ACCEPT].token_len);
	LOG("WSI_TOKEN_NONCE: %d\n", wsi->utf8_token[WSI_TOKEN_NONCE].token_len);
	LOG("WSI_TOKEN_PROTOCOL: %d\n", wsi->utf8_token[WSI_TOKEN_PROTOCOL].token_len);
#endif
	 if (!wsi->utf8_token[WSI_TOKEN_HTTP].token_len ||
		!wsi->utf8_token[WSI_TOKEN_UPGRADE].token_len ||
		!wsi->utf8_token[WSI_TOKEN_CONNECTION].token_len ||
	...
	if (strcmp(wsi->utf8_token[WSI_TOKEN_UPGRADE].token,
							 "websocket")) {
		LOG("libwebsocket_client_handshake server "
...
select_protocol:
	pc = wsi->c_protocol;
	if (pc == NULL)
		LOG("lws_client_interpret_server_handshake: NULL c_protocol\n");
	else
		LOG("lws_client_interpret_server_handshake: cPprotocol='%s'\n", pc);
	...
	if (!wsi->utf8_token[WSI_TOKEN_PROTOCOL].token_len) {
		...
		LOG("lws_client_interpret_server_handshake WSI_TOKEN_PROTOCOL is null\n");
		/*
	...
	if (!wsi->utf8_token[WSI_TOKEN_EXTENSIONS].token_len) {
		LOG("no client extenstions allowed by server \n");
		goto check_accept;
	}
	...
	while (more) {
		...
		while (ext && ext->callback) {
			...
			LOG("instantiating client ext %s\n", ext_name);
...
check_accept:
	...
	LOG("handshake OK for protocol %s\n",
						   wsi->protocol->name);
							  ...
							  struct pollfd *pollfd)
{
	...
	case LWS_CONNMODE_SERVER_LISTENER:
		...
		if (context->fds_count >= MAX_CLIENTS) {
			LOG("too busy to accept new client\n");
#ifdef WIN32
		...
		if ((context->protocols[0].callback)(context, wsi,
			...
			LOG("Callback denied network connection\n");
#ifdef WIN32
			closesocket(accept_fd);
#else
			close(accept_fd);
	...
	case LWS_CONNMODE_BROADCAST_PROXY_LISTENER:
		...
		if (context->fds_count >= MAX_CLIENTS) {
			LOG("too busy to accept new broadcast "
							      "proxy client\n");
#ifdef WIN32
			closesocket(accept_fd);
#else
	...
	case LWS_CONNMODE_BROADCAST_PROXY:
		...
		if (len < 0) {
			LOG("Error reading broadcast payload\n");
			break;
		}
	...
	case LWS_CONNMODE_WS_CLIENT_WAITING_PROXY_REPLY:
		...
		if (pollfd->revents & (POLLERR | POLLHUP)) {
			...
			LOG("Proxy connection %p (fd=%d) dead\n",
				(void *)wsi, pollfd->fd);
		...
		if (n < 0) {
			libwebsocket_close_and_free_session(context, wsi,
						     LWS_CLOSE_STATUS_NOSTATUS);
			LOG("ERROR reading from proxy socket\n");
			return 1;
		}
	...
	case LWS_CONNMODE_WS_CLIENT_ISSUE_HANDSHAKE:
		...
		if (n < 0) {
			LOG("ERROR writing to client socket\n");
			libwebsocket_close_and_free_session(context, wsi,
						     LWS_CLOSE_STATUS_NOSTATUS);
			return 1;
		}
	...
	case LWS_CONNMODE_WS_CLIENT_WAITING_SERVER_REPLY:
		...
		if (pollfd->revents & (POLLERR | POLLHUP)) {
			...
			LOG("Server connection %p (fd=%d) dead\n",
				(void *)wsi, pollfd->fd);
		...
		if (len < 0) {
			LOG("libwebsocket_client_handshake read error\n");
			goto bail3;
		}
...
bail3:
	...
	case LWS_CONNMODE_WS_CLIENT_WAITING_EXTENSION_CONNECT:
		LOG("LWS_CONNMODE_WS_CLIENT_WAITING_EXTENSION_CONNECT\n");
		break;

	case LWS_CONNMODE_WS_CLIENT_PENDING_CANDIDATE_CHILD:
		LOG("LWS_CONNMODE_WS_CLIENT_PENDING_CANDIDATE_CHILD\n");
		break;
	...
	case LWS_CONNMODE_WS_CLIENT:
		...
		if (pollfd->revents & (POLLERR | POLLHUP)) {
			...
			LOG("Session Socket %p (fd=%d) dead\n",
				(void *)wsi, pollfd->fd);
						       ...
						       struct libwebsocket *wsi)
{
	...
	if (n == context->fds_count)
		LOG("libwebsocket_callback_on_writable: failed to find socket %d\n", wsi->sock);
...
#if 0
	LOG("libwebsocket_rx_flow_control "
			       ...
			       int gid, int uid, unsigned int options)
{
	...
	if (!context) {
		LOG("No memory for websocket context\n");
		return NULL;
	}
	...
	if (context->fd_random < 0) {
		LOG("Unable to open random device %s %d\n",
	...
	if (p) {
		...
		if (p == NULL) {
			LOG("http_proxy needs to be ads:port\n");
			return NULL;
		}
		*p = '\0';
	...
	if (port) {
		...
		if (ssl_cert_filepath != NULL &&
					     ssl_private_key_filepath != NULL) {
			LOG(" Not compiled for OpenSSl support!\n");
			return NULL;
		}
		LOG(" Compiled without SSL support, "
						       "serving unencrypted\n");
#endif
	}
	...
	if (!method) {
		LOG("problem creating ssl method: %s\n",
			ERR_error_string(ERR_get_error(), ssl_err_buf));
		return NULL;
	}
	context->ssl_ctx = SSL_CTX_new(method);	/* create context */
	if (!context->ssl_ctx) {
		LOG("problem creating ssl context: %s\n",
			ERR_error_string(ERR_get_error(), ssl_err_buf));
		return NULL;
	}
	...
	if (port == CONTEXT_PORT_NO_LISTEN)
	{
		...
		if (!SSL_CTX_load_verify_locations(
			...
			LOG("Unable to load SSL Client certs from %s "
	...
	if (context->use_ssl) {
		...
		if (SSL_CTX_use_PrivateKey_file(context->ssl_ctx,
			...
			LOG("ssl problem getting key '%s': %s\n",
		...
		if (!SSL_CTX_check_private_key(context->ssl_ctx)) {
			LOG("Private SSL key doesn't match cert\n");
			return NULL;
		}
	...
	if (port) {
		...
		n = bind(sockfd, (struct sockaddr *) &serv_addr,
							     sizeof(serv_addr));
		if (n < 0) {
			LOG("ERROR on binding to port %d (%d %d)\n",
								port, n, errno);
			return NULL;
		}
