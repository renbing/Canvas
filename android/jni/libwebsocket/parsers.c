...
//	LOG("RX: %02X ", c);
	...
	case LWS_RXPS_NEW:
		...
		default:
			LOG("libwebsocket_rx_sm doesn't know "
...
handle_first:
		...
		if (c & 0x70)
			LOG("Frame has unknown extension bits set 1 %02X\n", c);
	...
	case LWS_RXPS_04_FRAME_HDR_LEN64_8:
		if (wsi->ietf_spec_revision < 7)
			c = wsi->xor_mask(wsi, c);
		if (c & 0x80) {
			LOG("b63 of length must be zero\n");
			/* kill the connection */
			return -1;
		}
#if defined __LP64__
...
spill:
		...
		case LWS_WS_OPCODE_07__CLOSE:
			...
			if (wsi->state == WSI_STATE_AWAITING_CLOSE_ACK) {
				 ...
				 * finish our close
				 */
				LOG("seen client close ack\n");
				return -1;
			}
			LOG("server sees client close packet\n");
		...
		if (wsi->protocol->callback)
			...
			LOG("No callback on payload spill!\n");
...
int libwebsocket_client_rx_sm(struct libwebsocket *wsi, unsigned char c)
{
	 ...
	 */
		 ...
		 */
			...
			if (c & 0x70)
				LOG("Frame has unknown extension "
		...
		default:
			LOG("client_rx_sm doesn't know how "
	...
	case LWS_RXPS_04_FRAME_HDR_LEN:
		...
		if ((c & 0x80) && wsi->ietf_spec_revision < 7) {
			LOG("Frame has extensions set illegally 4\n");
			/* kill the connection */
			return -1;
		}
...
spill:
		...
		case LWS_WS_OPCODE_07__CLOSE:
			...
			if (wsi->state == WSI_STATE_AWAITING_CLOSE_ACK) {
				 ...
				 * finish our close
				 */
				LOG("seen server's close ack\n");
				return -1;
			}
			LOG("client sees server close packet len = %d\n", wsi->rx_user_buffer_head);
			/* parrot the close packet payload back */
			n = libwebsocket_write(wsi, (unsigned char *)
			   &wsi->rx_user_buffer[LWS_SEND_BUFFER_PRE_PADDING],
				     wsi->rx_user_buffer_head, LWS_WRITE_CLOSE);
			LOG("client writing close ack returned %d\n", n);
			wsi->state = WSI_STATE_RETURNED_CLOSE_ALREADY;
			/* close the connection */
			return -1;
...
libwebsocket_0405_frame_mask_generate(struct libwebsocket *wsi)
{
	...
	if (n != 4) {
		LOG("Unable to read from random device %s %d\n",
...
//			LOG("ext sent it\n");
	...
	if (!wsi->sock) {
		LOG("** error 0 sock but expected to send\n");
	}
			  ...
			  size_t len, enum libwebsocket_write_protocol protocol)
{
	...
	if (len == 0 && protocol != LWS_WRITE_CLOSE) {
		LOG("zero length libwebsocket_write attempt\n");
		return 0;
	}
	...
	case 6:
		...
		case LWS_WRITE_CLOSE:
			...
			case 5:
				...
				case LWS_CONNMODE_WS_SERVING:
					/*
					LOG("LWS_WRITE_CLOSE S\n");
					*/
					buf[0] = 'S';
					break;
				case LWS_CONNMODE_WS_CLIENT:
					/*
					LOG("LWS_WRITE_CLOSE C\n");
					*/
					buf[0] = 'C';
					break;
				default:
		...
		case LWS_WRITE_PONG:
			...
			break;
		default:
			LOG("libwebsocket_write: unknown write "
							 "opcode / protocol\n");
			return -1;
		}
